...
> **最新三次真机验收（第十轮）**：① 审批卡片上真的出现三个按钮（批准 / 拒绝 / 跳转到Harness），
> 用户点「批准」后，那次需要审批的写入**成功落盘** —— 应答者、协议激活、启动器 `/decide`、
> 令牌兑现这条链一次走通。② 修掉「通知批准后界面弹框还挂着」之后，用户在通知上点「批准」→
> 写入成功且**界面没有再出现弹框** ✓。③ 修掉「点跳转后批准界面延迟数秒」之后，
...
| 「发送这条通知」：删掉通用的「发一条测试通知」，改到「通知内容」卡片里**按当前场景**真发一条 | 客户端已生效；**Host 侧的 `/notify` 与 `bypass` 需重启** | 行为审计断言「发出的正文与预览区那一行逐字相同」+「结果必须报在按钮那一行」；`npm test` 5 条 bypass/400 断言 |
| 通知**最上方那一行**显示配置里的通知署名（用户看通知时把它当标题，而它其实是 Windows 按 AUMID 显示名渲染的应用名） | 客户端已生效；Host 侧的同步需重启 | 实验确认（改注册表值 → 那一行就变了）；`post-restart-check.mjs` 有可机器判定的比对 |
| 审批卡片三个按钮（批准 / 拒绝 / 跳转到Harness），批准与拒绝**真的提交决定** | 启动器已重建；**Host 侧需重启** | ADR 0007；
pm test 88 条（令牌一次性/过期/沉默不是同意/不泄露令牌）+ vents-wiring-check |
| 署名**只**出现在标题上：toast 不再写标题行，正文也去掉了 `{workspace}` 前缀 | 标题行：Host 侧需重启；正文前缀：**已当场生效**（模板是配置驱动的） | `npm test` 断言默认正文不含 `{workspace}`；改用户模板后回读确认 |
| 通知图标换成 DeepSeek Harness 自己的图标 | **已当场生效**（注册表 `IconUri` + 快捷方式图标）；toast 的 `appLogoOverride` 需重启 | `post-restart-check.mjs` 比对 `IconUri`；`npm test` 断言图标资源格式与脚本里的守卫 |

**关于「不要重启」**：署名的同步本来就是「保存即同步」，装好之后**改署名是立即生效的**。
需要重启的只有**一次性安装**——Host 半边在 DSH 进程里，而改动发生在进程启动之后。
本轮实测确认**重挂载插件不能替代重启**：禁用再启用 `include:session-alert` 后插件确实重新挂载
（`/state` 从 401 恢复到 200），但跑的还是旧代码——cordis Loader 复用 Node 的 ESM 缓存。
