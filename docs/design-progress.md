...
## 六、弹出流程：已收敛为两条流程，且全部通过

用户最终把验收范围收窄为**两条流程**，其余（命令行闪烁、窗口缩小等）都是这两条的
故障表现，不再单列：

| 流程 | 基准尺寸 | 实际尺寸 | 尺寸未变 | 命令行闪烁 |
| --- | --- | --- | --- | --- |
| 1 最小化 → 弹出到前台 | 1721x927 | 1721x927 | ✅ | 0 |
| 2 完全隐藏 → 弹出到前台 | 1721x927 | 1721x927 | ✅ | 0 |

连续三次运行结果一致。测试：[`experiments/popup-two-flows.ps1`](../experiments/popup-two-flows.ps1)。

### 过程中定位到的真实缺陷（都已修）

1. **`SW_RESTORE` 会把最大化窗口缩小。** `SW_RESTORE` 的语义是「还原」，对最大化窗口
   调用它会缩回最大化之前的普通尺寸。应按形态分派。
2. **`SW_SHOW` 会把隐藏的最大化窗口降级成普通大小。** 实测 1721x927 → 1279x671。
   隐藏不会抹掉最大化状态，因此需先显示、再按原形态修正。
3. **`Process.MainWindowHandle` 在窗口隐藏时为 0**，它只报告可见的主窗口。用户明确要求
   覆盖「应用从任务栏完全消失」，而这一定位方式会把该场景误判为「窗口已销毁」。
   必须改用 `EnumWindows` 按进程枚举。

### 测试自身踩过的坑（都已修，值得记住）

- **两套占位符索引混用**：协议激活的 `%1`（系统填 URL）与 PowerShell `-f` 的 `{0}`
  是两回事，混用会让参数一直是字面量。
- **窗口形态转换是异步且有竞态的**：对齐形态必须**重试动作本身**，只调一次再等会偶发
  失败；用例之间还必须彻底复位到已知可见状态，否则残留形态会污染后续结论。
  为此加了「前置条件检查」——探针必须报告出预期形态，否则该用例结论作废。
- **被 `catch` 吞掉的异常极难归因**：重写脚本时漏掉一个 `DllImport` 声明，调用处抛异常
  被末尾 `catch` 吞掉只留一行日志，外表表现为「探针没生效」。为此加了前置自检，显式
  校验每个要用的 P/Invoke 入口点都存在，缺失即报错退出。
- **`$pid` 是 PowerShell 只读自动变量**，覆盖它会让赋值静默失败。
- **`$attempt:` 会被当成驱动器限定变量**（像 `$env:`），需写成 `${attempt}`。
- **控制台窗口的关闭是异步的**：测量窗口之间必须留足排空时间，否则上一个用例正在关闭的
  窗口会被计入下一个用例，造成计数漂移。
