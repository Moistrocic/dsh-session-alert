...
## 六、还开着的一个问题：用户报告的那个闪窗到底来自谁

用户报告「切换窗口时会弹出一个命令行界面」。已把三条候选路径逐个钉死：

| 路径 | 验证方式 | 结论 |
| --- | --- | --- |
| 插件**发通知**时的 spawn | 用户人工确认 + 监视器 | 不闪（`NO_CONSOLE`） |
| 协议激活，旧写法 `-WindowStyle Hidden` | 用户人工确认 + A/B 对照 | **会闪**，已定位并给出合格方案 |
| 协议激活，新写法 `conhost --headless` | A/B 对照 | 不闪 |

**但用户无法复述该现象出现在什么操作之后**，所以还剩下一种可能没排除：它来自**第三方**
（另一个插件的定时任务、或 DSH 自身某处的进程启动），与本插件无关。

工具已就绪：[`experiments/console-watch.ps1`](../experiments/console-watch.ps1) 监视
**可见的**控制台窗口并报出所属进程与父进程，因此让用户照常使用、事后读日志即可归因，
不必靠肉眼捕捉一闪。

**排查时的一个陷阱（已实测踩到）：** 被捕捉到的窗口属于 `WindowsTerminal.exe`，其父进程
是 `svchost.exe`，**不是**发起调用的进程——现代 Windows 会把控制台托管给 Windows
Terminal。因此**不能靠父进程归因**，必须看窗口可见性与时间戳。
