...
用户反馈两条，第二条是 0.2 第一版引入的**新缺陷**：

1. 点「跳转到Harness」后，界面批准提示**至少延迟 5 秒**才出现（0.1 的宽限期代价）。
2. 「**DSH 在前台时弹出的通知，点批准依旧无效**」。
...
修法（都在 `lib/index.js`）：
- 抽出 `isDesktopFocused()`（只回答「此刻是否在前台」，不含配置判断）；
- 交接条件收紧为「**发出时不在前台、之后才获得焦点**」：发出时已在前台就不启用焦点交接
  （`alreadyFocused ? null : desktopFocusSignal()`）；
- 等待里并列的焦点分支加 `focus !== null` 守卫。

> **状态：真机观察验收中。** 第 1 条已由用户确认通过（点跳转 → 提示已在）。
> 第 2 条刚改完，**未验证**；测试套件对这几处改动一次都没跑过（shell 不可用），也未提交。
> **验收判据（第 2 条）**：DSH 在前台时收到审批通知 → 点通知上的「批准」应当**有效**
> （操作通过）；同时点「跳转到Harness」那条不能退化。

用户反馈：点「跳转到Harness」之后，界面的批准提示**至少延迟 5 秒**才出现。

原因是 0.1 那次改动的代价：交替式之后，界面提示只在宽限期（20 秒）到、或中止时才创建；
而「跳转到Harness」**不消费令牌** —— 用户点了它、窗口跳到眼前，提示却还没出生，
他只能干等宽限期的尾巴。

修法：把「desktop 端获得焦点」也当成一次交接触发。客户端半边的焦点上报是**事件驱动**的
（`focus`/`blur` 监听后立即上报，见 `lib/client.js`），所以交接是几十毫秒级，而不是等心跳。
改动只在 `lib/index.js`：`desktopFocusSignal()` ＋ 在审批等待里并列一个焦点分支
（`finally` 里 `cancel()`，否则等待器会留在集合里）。

> **状态：真机观察验收通过；测试套件仍未跑。** 当时 shell 不可用
> （`workspace-write` 下沙箱设不了工作区 ACL），连 `node --check` 都跑不了；改动未提交。
>
> **真机验收（用户确认）**：触发一次审批 → 用户点「**跳转到Harness**」→ 窗口跳到眼前时
> **批准提示已经在**（不再是数秒延迟）✓。也就是说「desktop 端获得焦点 → 立刻交接」
> 这条信号真的通到了 Host。
>
> **仍未验的**：通知上点「拒绝」（fail closed 那一路的端到端）、以及「20 秒不点、由宽限期
> 兜底」这两条仍未在真机上验过；`npm test` 与 `node experiments/events-wiring-check.mjs`
> 一次都没跑过（这两处改动都没有）。

## 0.1 修「通知批准后界面弹框还在」（**真机观察验收通过；测试套件仍未跑**）

用户反馈：在通知上点了「批准」，**实际界面上依然显示批准对话框**，分不清是「没生效」
还是「没更新」。查证：批准**生效了**（那次需要审批的写入成功落盘），是弹框没被收回 ——
见上面第 0 节末尾与 [ADR 0007](./adr/0007-approval-controls-submit-decisions.md) 里
「为什么是交替而不是两边同时」那一节。

改动：`lib/index.js` 的审批监听器从「并行 + race」改成「**先只等通知（宽限期 20 秒）
→ 超时才交界面**」，超时同时作废令牌；`scripts/event-harness.mjs` 把宽限期压到 0
（否则接线检查要白等 20 秒）。
...
> **仍未做的**：`npm test` 与 `node experiments/events-wiring-check.mjs` **一次都没跑过**
> 这次改动（当时 `workspace-write` 下沙箱设不了工作区 ACL，`pwsh` 直接失败；唯一一次升级
> 请求被拒）。所以这条的凭据是**真机观察**，不是测试套件 —— 接手时请补跑，尤其是接线检查
> （它现在要 await 监听器之后才数 `next()` 调用次数）。改动**未提交**（git 需要 shell）。

用户的四条要求：

1. 点卡片不触发任何逻辑，只确认收到；
2. 默认卡片加一个「跳转到Harness」按钮；
3. 审批通知三个按钮：**批准 / 拒绝 / 跳转到Harness**（顺序即左右）；
4. （隐含，也是本项目的一条硬规矩）文案必须与实际行为一致 —— 批准就得真的批准。

## 0. 真机验收（当轮完成）

用户把审批策略从 `never` 改成 `ask` 之后，做了一次**真实的端到端验证**：
...
并让操作真的通过。**「拒绝」那一路未做同样的端到端验证**：它的分支逻辑有单测
（`deny → rejected`），点击管线与「批准」完全相同，只差一个决定词。

当时环境的一个限制值得记下：`workspace-write` 策略下本机沙箱**无法为工作区设置 ACL**
（`SetNamedSecurityInfoW failed (Win32 5)`），因此 `pwsh` 一类命令直接失败；
工作区内的文件工具不受影响。那次 `/state` 读数因此拿不到，验证完全依靠
「用户看到了什么」＋「文件是否落盘」这两条可观测事实。

## 一、点卡片什么都不做

toast 元素加 `activationType="system"`。此前不写这个属性时，点卡片会按 AUMID 激活应用身份，
也就是拉起开始菜单快捷方式指向的那个「惰性命令」（实测是一次隐藏的 PowerShell 进程）——
不是「什么都没有」，只是看不见。加了属性之后系统只是收下这条通知。
实测：XML 仍被接受（投递退出码 0 = 已显示并在操作中心里回读到）。

## 二、跳转从「点卡片」移到按钮

ADR 0002 当年让卡片本身承担跳转；现在卡片不做事了，跳转改成一个明确的按钮
（`dsh-session-alert://open/?session=…`）。普通卡片 = 一个「跳转到Harness」，
旧的「知道了」与「去处理…」都不再有。

判决串同时改成带**按钮个数**（`card+1button` / `card+3buttons`）：卡片形状是要一眼看出来的
诊断信息，合成一个 `card+button` 就分不清审批卡片与普通卡片了。

## 三、审批真的能批准（新增 ADR 0007，取代 ADR 0006 的结论）

关键查证：**DSH 的审批应答者就是 `approval/request` 的 waterfall 监听器**
（`@deepseek-ai/dsh-user-approval` 文档原话；事件目录里的签名也证实了）。
插件作为应答者被询问时**就在那个开放轮次内**——ADR 0006 说的「轮次之外的外部进程」
对通知投递与点击成立，对**应答者**不成立。ADR 0006 否决的另一条路（把请求 id 放进 URL、
让外部进程稍后回调）依然有效，而且**没有被采用**。

机制（`lib/decisions.js` + `lib/index.js` + 启动器）：

```
approval/request 监听器
  ├─ 开一个待决令牌（128 位随机、一次性、10 分钟有效）
  ├─ 立刻调用 next()：界面那侧的提示照常出现
  ├─ 发通知：批准 / 拒绝 两个按钮带令牌，第三个按钮跳转
  └─ return await Promise.race([令牌 promise, 界面 promise])   ← 谁先给决定就采纳谁

点「批准」→ 协议激活 → 启动器 → POST /decide {token, decision} → 令牌兑现 → 返回 allowed-once
```

启动器新增 `decide` 分支：**不碰窗口**，只把 token/decision POST 回宿主；失败时弹一个
提示框——用户点了「批准」却什么都没发生，是最糟的结果（他会以为已经批了）。

### 六条不变式（每条都有断言）

1. 一个令牌绑定一个请求，且只能用一次（重放被拒，绝不落到别的请求上）；
2. 过期即失效（10 分钟），且**过期绝不产生决定**；
3. **沉默绝不是同意**：拿不到决定就交回界面，没有默认放行也没有默认拒绝；
4. 卡片没发出去（被抑制／被跳过）就立刻作废令牌——按钮不在，令牌不该还活着；
5. 令牌不落盘、**不进 `/state`**（`/state` 本机无凭据可读）；
6. `/decide` 无凭据，令牌是唯一的关（128 位随机，只出现在用户点得到的那张卡片里）。

## 顺带修掉两处测试基础设施的坑

1. **`client-behavior-audit` 改写了全局却从不还原。** 它接管 `setTimeout`/`clearTimeout`/`fetch`
   来驱动防抖，代码里有个 `saved` 映射——**但从来没有用它还原**。而 `scripts/selftest.mjs`
   是 in-process 导入它的，于是那之后的所有用例都在用**永不触发的假计时器**：
   本轮新加的一条 10ms 计时器断言因此静默挂死（整份测试卡到超时才发现）。
   现在模块加载时记下原件，`finally` 里还原。
2. **`npm test` 加了 `--test-timeout=60000`。** 一个卡死的用例应当**失败**并指出自己，
   而不是把整份测试挂住——刚才就是靠它定位到第 1 条的。

另外两处判据修正：`listener-mode-audit` 从「必须 `return next()`」放宽为「必须调用
`next()`」（瀑布监听器可以**先接管、再交出决定权**，审批场景正是如此）；
`events-wiring-check` 相应地先 await 监听器的返回值再比对（契约本身就是
`Promise<ApprovalOutcome>`，直接比字符串会把正确实现判成失败）。

`npm test` 78 → **88 条**（令牌表 5 条、按钮 URL、按钮顺序、决定路由、`/state` 不泄露令牌、
卡片 `activationType`）。

---

# 2026-09-25 第九轮：内容里多出来的那个图标

用户反馈：「标题的图标确实正常，但是内容为什么还有一个图标？图标只需要标题左侧就行了。」

两个图标来自两处**不同**的机制：

| 位置 | 来源 | 处理 |
| --- | --- | --- |
| 标题左侧（应用名旁） | **应用身份**：注册表 `IconUri` + 开始菜单快捷方式图标 | 保留 —— 这正是用户要的 |
| 正文里 | toast XML 的 `<image placement="appLogoOverride">` | **移除** |

`appLogoOverride` 看着像「给应用换个图标」，实际效果是**在正文区域再画一个图标**。
应用图标本来就已经显示在标题左侧了，所以它只会多出一个。

改动：

- `buildToastScript` 不再接受 `iconPath`，生成的 XML 里**没有任何 `<image>` 元素**；
  `sendWindowsToast` 与宿主的 `sendExtras` 同步去掉该字段。
- `scripts/build-notification-icon.ps1` 只生成多尺寸 ICO（注册表与快捷方式用），
  不再生成给 toast 用的 PNG；`assets/notification-icon.png` 已删除。
- 自检改成**反向断言**：生成的脚本里不得出现 `CreateElement('image')` / `appLogoOverride` /
  `file:///`，同时正文与标题节点照旧写出（别为了删图标把内容一起删了）。

这条属 Host 侧改动，**下次重启生效**（纯外观，不急）。
...
1. **Host 半边还没重启**，`/notify` 回 404（运行中的进程是 17:45 启动的，而端点是之后加的）。
2. **更糟的是：结果报在了页面底部的「操作」那一行。** 按钮在中间的「通知内容」卡片里，
   提示在页面最下方——用户点了按钮，结果出现在屏幕外，**等于没显示**。

第 2 条是我的实现缺陷，与重启无关：把结果显示在动作之外的地方，与不显示没有区别。
修法：

- `notice` 带一个 `where`：发送预览的结果报在**按钮所在的那一行**（`where: 'editor'`），
  自动保存的状态留在底部（保存说明在那里）。
- 404 不再只报「未知端点」，而是直说**下一步该做什么**：
  「Host 半边还是旧代码——这个入口要重启 DSH 才会生效。」404 在这个端点上只可能是这个成因。
- 其余失败原因仍原样带出来（Host 会说明是总开关关着还是正文太长）。

行为审计相应加强，并且**加了祖先链**：断言「结果出现在发送按钮所在的那一行」，
而不只是「页面上有这段文本」——后者抓不到这类缺陷。同时补了一条 404 场景的断言
（提示必须出现在按钮旁、且必须提到「重启」）。变异检查新增一条：把提示挪回页面底部，
必须被抓出来。

## 这一轮暴露的两处「审计自身的错」（都连报了两轮假失败）

1. **`rerender()` 只渲染、不更新 `tree`**，于是「读回界面」的断言一直在看旧树，
   报出来的是「结果没有显示在按钮那一行（页面上它出现在 null）」——看着像产品没渲染。
2. **节点身份跨遍历不成立**：`walk` 每次遇到函数型组件都会重新渲染它，
   **每次遍历产生的节点对象都是新的**，拿上一次遍历定位到的「行」去比这一次的祖先链永远不命中。
   改成**在同一次遍历里**同时收集「按钮所在的行」与「提示」，并按祖先链做包含判断。

两条都属于同一句话：**读回界面时，先确认自己读的是刚刚渲染的那棵树。**
