...
### 二、前台锁的拒绝是预期的，必须如实呈现而不是掩盖

**实测（同一 helper、同一窗口状态，唯一变量是“DSH 是否为最近使用的应用”）：**
...
这是 Windows 前台锁的**设计行为**——它的目的就是不打断用户当前正在做的事
（实测场景：用户在 Chrome 里看视频）。**没有正当手段可以绕过，也不应该尝试绕过。**
七种手法实测全部无效：`SetForegroundWindow` 直调、`AllowSetForegroundWindow` 后再调、
先最小化再还原、`SetWindowPos(HWND_TOPMOST, SWP_NOACTIVATE)`、`BringWindowToTop` 循环、
`AttachThreadInput` 窃取前台线程（返回 `False`）、`SwitchToThisWindow`。
