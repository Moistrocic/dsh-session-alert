		...
		function SessionAlertSection() {
			...
			function sendPreview(scenarioId, body) {
				...
				if (text.length === 0) {
					...
					setNotice({ kind: 'warn', text: tx('previewEmpty') })
					return
				}
				setBusy(true)
				setNotice(null)
				try {
					fetch(NOTIFY_ENDPOINT, {
						...
						.then(function (result) {
							...
							if (payload.ok === true) {
								setNotice({ kind: 'ok', text: tx('previewSent') })
								return
							}
							// 失败原因原样带出来：Host 会说明是「通知总开关关着」还是「正文太长」，
							// 含糊其辞会让用户以为按钮坏了。
							var reason = payload.error !== undefined
								...
								: (payload.outcome !== undefined && payload.outcome !== null && payload.outcome.reason !== undefined
									...
									: tx('unknown'))
							setNotice({ kind: 'error', text: tx('sendFailed') + reason })
						})
						.catch(function (cause) {
							setBusy(false)
