...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 发送一条「这个场景长什么样」的预览。正文由本侧给出（就是预览区那一行）。 */
		var NOTIFY_ENDPOINT = '/api/dsh-session-alert/notify'
		...
		/**
		 * 界面文本（中文基准表，也是 locale 服务不可用时的本地兜底）。
		 *
		 * **为什么要在 `en` 下也注册一份**：DSH 内置语言只有 `zh` 与 `en`，且 `en` 是
		 * 兜底语言——查不到的键会退到它。如果只注册 `zh`，那么当活动语言是 `en` 时，
		 * 所有键都会 miss 并退化成把 key 本身显示出来（界面上会出现一排 ID）。
		 *
		 * **`en` 与 `zh` 的差别刻意只有一处**：设置页的标签名（`title`）。
		 * 它是 DSH 设置页里**与其它插件并列显示**的名字，用户在语言切换时期待它跟着变，
		 * 所以中文用「会话通知」、其余语言用「Session Alert」。
		 *
		 * 其余键两种语言下都是中文，理由是**界面语言与通知内容是两件事**：本插件的
		 * 通知模板本身是中文的（默认模板、`{workspace}` 这类占位符、示例数据都是），
		 * 界面切成英文而通知仍是中文会更怪；DSH 自己的设置页在这个语言下的键也是中文的。
		 * 这也意味着：**将来要真正做英文界面，改的是这张表加一份覆盖表，不是改结构**。
		 */
		var TEXTS = {
			title: '会话通知',
			...
			notifyTitle: '通知署名',
			notifyTitleHint: '显示在通知最上方那一行（Windows 按应用名渲染它），也会出现在系统的通知设置里。改动会自动同步到系统。',
			withSound: '通知带声音',
			rootOnly: '只提醒根会话',
			...
			ratePendingUnit: ' 条',
			rateSecondsUnit: ' 秒）',
			// **没有「保存」按钮了**：改动自动保存（见组件里的自动保存效应）。
			// 因此这里不再有 `save` / `saving` 两个键——留着它们只会让人以为还有个按钮。
			autoSaveHint: '改动会自动保存（切换或关闭设置页时立即写入）。',
			autoSaving: '正在保存…',
			// 「发送这条通知」：在「通知内容」卡片里，按**当前选中的场景**真发一条。
			// 它取代了原先通用的一条「发一条测试通知」——用户要看的是这一种长什么样，
			// 而不是一条与任何场景都无关的样例。
			sendPreview: '发送这条通知',
			previewSending: '发送中…',
			previewSent: '已发送这条预览。',
			// 投递链没在时限内回来。**不报失败**：慢不等于失败，报失败会让人去查没坏的地方。
			previewPending: '已发出，但没等到投递结果（可能只是慢）。可看下面「最近发出的通知」。',
			// 404 只有一种成因：**Host 半边还是旧代码**（端点是在新代码里加的），
			// 而 Host 半边改了必须重启 DSH 才进进程。含糊地报「未知端点」会让人以为是插件坏了。
			previewNeedsRestart: 'Host 半边还是旧代码——这个入口要重启 DSH 才会生效。',
			saved: '已自动保存。',
			...
			aumidDisplayName: '通知最上方那行',
			aumidSynced: '（已同步为上面的通知署名）',
			aumidNotSynced: '仍是注册名（署名还没同步成功，因此通知里保留了标题行）',
			configPath: '配置文件',
			recentSignals: '最近的信号（',
			recentSignalsUnit: ' 条）',
			noSignals: '还没有任何信号到达。',
			recentToasts: '最近发出的通知',
			noToasts: '还没有发出过通知。可以在「通知内容」里点「发送这条通知」验证投递链。',
		...
		/**
		 * **中文以外**的语言下与 `TEXTS` 不同的键。
		 *
		 * 目前只有一处：设置页的标签名。它是 DSH 设置页里与其它插件并列显示的名字，
		 * 用户在语言切换时期待它跟着变；而 DSH 只有 `zh` 与 `en`，且 `en` 是兜底语言，
		 * 所以「其他语言 = 英文」这一条由 `en` 这一份字典承担。
		 *
		 * 其余键不做英文，理由写在 `TEXTS` 上方：通知模板本身是中文的。
		 */
		var TEXTS_EN_OVERRIDES = {
			title: 'Session Alert',
		}
		 ...
		 * 自动保存的防抖时长（毫秒）。
		 *
		 * 为什么要防抖而不是「一变就存」：用户在数字框里把 30 打成 3 的那一瞬间也是一次
		 * 合法改动，立刻落盘会把「3」写进配置文件。防抖让一次连续编辑只落一次盘。
		 * 这个值只影响「编辑停止后多久落盘」；**切换或关闭设置页时会立刻落盘**，不受它影响。
		 */
		...
		function TemplateEditor(props) {
			...
			return h('div', { className: 'dsa-editor' }, [
				...
				// 预览下面就是「按这条发一条」——用户在预览里看到什么，点下去就发什么。
				// 它取代了原先那条与场景无关的「发一条测试通知」。
				//
				// **结果就报在这里**（`where: 'editor'`），不是页面底部那一行：
				// 点了按钮却在屏幕外显示结果，等于没显示。
				h('div', { key: 'editor-actions', className: 'dsa-actions' }, [
					h('button', {
						key: 'send',
						type: 'button',
						...
						onClick: function () {
							props.onSend(active, renderPreview(scenarioConfig.body, active))
						},
					}, props.sending === true ? tx('previewSending') : tx('sendPreview')),
					h('button', {
						key: 'reset',
						type: 'button',
					...
					props.notice !== null && props.notice !== undefined && props.notice.where === 'editor'
						? h('span', {
							key: 'notice',
							className: 'dsa-notice'
								+ (props.notice.kind === 'error' ? ' dsa-notice-error' : (props.notice.kind === 'warn' ? ' dsa-notice-warn' : '')),
						}, props.notice.text)
						: null,
				]),
			])
		...
		function SessionAlertSection() {
			...
			// ---- 自动保存用的引用 ----
			//
			// **必须放 ref 而不是 state。** 卸载时的清理函数与 `pagehide` 监听器是**第一次
			// 渲染**那次闭包创建的，它们读 state 只会读到装载时的旧值；ref 读到的才是最新值。
			// 「切换/关闭设置页时把改动补上」能成立，靠的就是这一点。
			var draftRef = useRef(null)      // 最新草稿
			var lastSentRef = useRef(null)   // 最近一次成功落盘的内容（规范化字符串）
			var timerRef = useRef(0)         // 防抖计时器
			var inFlightRef = useRef(false)  // 是否已有一次保存在途中
			var queuedRef = useRef(false)    // 保存途中又改了 → 这次回来后再存一次
			var aliveRef = useRef(true)      // 组件是否还挂着（卸载后不再动界面状态）
				...
				try {
					fetch(STATE_ENDPOINT)
						...
						.then(function (body) {
							if (cancelled) return
							setSnapshot(body)
							// **装载不算「改动」**：不记下来的话，草稿一填上就会被自动保存判成
							// 「有新内容要写」，于是每次打开设置页都白写一次盘。
							lastSentRef.current = stableJson(body.config)
			...
			/**
			 * 键顺序无关的序列化，用来比较「有没有新改动」。
			 *
			 * 直接用 `JSON.stringify` 依赖键的插入顺序：服务端归一化后重建的对象键序可能与
			 * 草稿不同，于是「内容其实一样」会被判成「改过了」，**保存按钮没了以后这会变成
			 * 一个自己咬自己的循环**（存完发现「又有改动」→ 再存）。排序后比较就没有这个问题。
			 *
			 * @param value - 任意可序列化值。
			 * @returns 稳定的字符串。
			 */
			...
			/**
			 * 把当前草稿写回 Host。**这是自动保存的唯一出口**（没有保存按钮了）。
			 *
			 * 三种时机都会走到这里：编辑停止 `AUTOSAVE_DEBOUNCE_MS` 之后、组件卸载
			 * （切换或关闭设置页）、以及页面 `pagehide`（应用被关掉）。
			 *
			 * @param options - `{ ui: false }` 表示不要动界面状态（卸载/页面关闭时用）。
			 */
			function flushSave(options) {
				var showUi = !(options !== undefined && options !== null && options.ui === false)
				var payload = draftRef.current
				if (payload === null || payload === undefined) return
				var sent = stableJson(payload)
				// 与上次落盘的内容一致就什么都不做：装载、服务端回填都会走到这里。
				if (sent === lastSentRef.current) return
				if (inFlightRef.current) { queuedRef.current = true; return }
				inFlightRef.current = true
				if (showUi && aliveRef.current) setNotice({ kind: 'ok', text: tx('autoSaving') })
				try {
					fetch(CONFIG_ENDPOINT, {
						...
						.then(function (result) {
							finishSave()
							var normalized = result.body !== undefined && result.body !== null ? result.body.config : undefined
							if (normalized !== undefined) {
								setSnapshot(function (previous) {
									if (previous === null) return previous
									var next = {}
									for (var k in previous) if (Object.prototype.hasOwnProperty.call(previous, k)) next[k] = previous[k]
									next.config = normalized
									return next
								})
								// 服务端会归一化（例如把 999 钳成 60）。**只在用户没有再改的前提下回填**——
								// 否则会覆盖他正在输入的内容，而那正是「自动保存」最容易造成的伤害。
								if (stableJson(normalized) !== sent && stableJson(draftRef.current) === sent) {
									setDraft(normalized)
								}
								lastSentRef.current = stableJson(normalized)
							} else {
					...
					finishSave()
					if (aliveRef.current) setNotice({ kind: 'error', text: String(cause) })
				}
			}

			// 草稿一变：记进 ref 并重置防抖计时器。
			//
			// **这个效应刻意不返回清理函数**：清理会在每次草稿变化前跑一遍，而卸载时也要跑，
			// 两种时机的处理完全不同（前者只是重置计时器，后者必须立刻落盘）。
			// 硬凑成一个清理函数会让「改一下就打一次盘」。
			useEffect(function () {
				draftRef.current = draft
				if (draft === null) return
				if (stableJson(draft) === lastSentRef.current) return
			...
			// 卸载 = 用户切换到了别的设置分区、或关掉了设置面板。
			// **这正是「切换或关闭设置标签页时自动保存」那一刻**：把还没落盘的改动立刻写回。
			useEffect(function () {
				return function () {
			...
			// 兜底：应用被关掉时防抖计时器可能来不及触发。
			useEffect(function () {
				if (typeof window === 'undefined' || typeof window.addEventListener !== 'function') {
					return function () { /* 没有 window 就没有要摘的监听器 */ }
				}
				function onPageHide() {
					if (timerRef.current !== 0) { clearTimeout(timerRef.current); timerRef.current = 0 }
					flushSave({ ui: false })
			 ...
			 * 发送「这一种通知长什么样」的预览。
			 *
			 * **正文由本侧给出**（就是预览区显示的那一行），因此「看到什么就发什么」是
			 * 由构造保证的：不必假设客户端的预览渲染与 Host 的模板渲染会碰巧一致。
			 * Host 那侧只负责策略与投递，并且这次投递绕过限流/去重/抑制——用户当场按下的
			 * 动作被静默扣下，只会让他以为按钮坏了。
			 *
			 * **所有结果都报在按钮旁边**（`where: 'editor'`）。用户点的是上面的按钮，
			 * 把结果显示在页面底部的「操作」那一行等于没显示——实测就是这样被反馈
			 * 「按了没反应」的。
			 *
			 * @param scenarioId - 当前选中的场景 id。
			 * @param body - 预览区里渲染好的正文。
			 */
			function sendPreview(scenarioId, body) {
				var text = typeof body === 'string' ? body.trim() : ''
				if (text.length === 0) {
					// 空模板渲染出来就是空。如实回绝，而不是发一条空白通知。
					setNotice({ kind: 'warn', text: tx('previewEmpty'), where: 'editor' })
					return
				}
				setBusy(true)
				setNotice(null)
				try {
					fetch(NOTIFY_ENDPOINT, {
						method: 'POST',
						headers: { 'content-type': 'application/json' },
						body: JSON.stringify({ scenario: scenarioId, body: text }),
					})
						.then(function (response) {
							return response.json().then(function (payload) { return { status: response.status, ok: response.ok, body: payload } })
						})
						.then(function (result) {
							setBusy(false)
							var payload = result.body !== undefined && result.body !== null ? result.body : {}
							if (payload.ok === true) {
								// 把走通的投递路径一并说出来：那是「横幅到底有没有出来」的唯一信号。
								var via = payload.via !== undefined && payload.via !== null ? String(payload.via) : ''
								setNotice({
									kind: 'ok',
									text: via.length > 0 ? tx('previewSent') + via : tx('previewSent'),
									where: 'editor',
								})
								return
							}
							if (payload.pending === true) {
								// 超时：投递链还没回来。**不报失败**——超时不等于失败，投递可能只是慢。
								setNotice({ kind: 'warn', text: tx('previewPending'), where: 'editor' })
								return
							}
							// 失败原因原样带出来，并且**说清下一步该做什么**：
							// 404 只可能是 Host 半边还没重启（端点在新代码里）；其余情况 Host 会
							// 说明是「通知总开关关着」还是「正文太长」。含糊其辞会让人以为按钮坏了。
							if (result.status === 404) {
								setNotice({ kind: 'error', text: tx('previewNeedsRestart'), where: 'editor' })
								return
							}
							var reason = payload.error !== undefined
								? String(payload.error)
								: (payload.outcome !== undefined && payload.outcome !== null && payload.outcome.reason !== undefined
									? String(payload.outcome.reason)
									: tx('unknown'))
							setNotice({ kind: 'error', text: tx('sendFailed') + reason, where: 'editor' })
						})
						.catch(function (cause) {
							setBusy(false)
							setNotice({ kind: 'error', text: tx('sendFailed') + String(cause && cause.message ? cause.message : cause), where: 'editor' })
						})
				} catch (cause) {
					setBusy(false)
					setNotice({ kind: 'error', text: String(cause), where: 'editor' })
				}
			}
			...
			return h('div', { className: 'dsa-root' }, [
				...
				h('section', { key: 'general', className: 'dsa-card' }, [
					...
					h('label', { key: 'title', className: 'dsa-field' }, [
						...
						// 说明这一栏**到底改的是什么**：用户看通知时把最上面那一行当成标题，
						// 而那一行由 Windows 按 AUMID 的显示名渲染，不是 toast XML 决定的。
						// 不说清楚，用户会以为改了没生效（这确实是本插件踩过的坑）。
						h('span', { key: 'h', className: 'dsa-hint' }, tx('notifyTitleHint')),
					]),
					h(Toggle, {
						key: 'sound',
						label: tx('withSound'),
				...
				// ---- 操作 ----
				//
				// **这里既没有「保存」按钮，也没有「发一条测试通知」**：
				//  - 改动自动保存（见组件上方那三个效应）；
				//  - 发通知的入口在「通知内容」卡片里、预览的正下方（「发送这条通知」），
				//    按当前选中的场景真发一条——用户要看的是这一种长什么样，
				//    而不是一条与任何场景都无关的样例。
				//
				// 这一行因此只剩说明与状态：没有按钮就得说清楚改动会自己保存，
				// 否则用户会去找保存按钮，或者不敢关掉设置页。
				// 这一行只报**保存**相关的状态（说明与告警）。发送预览的结果报在按钮旁边
				// （见「通知内容」卡片），不在这里——点了按钮却在屏幕外显示结果等于没显示。
				h('div', { key: 'actions', className: 'dsa-actions' }, [
					h('span', { key: 'auto', className: 'dsa-hint' }, tx('autoSaveHint')),
					notice !== null && notice.where !== 'editor'
						? h('span', {
							key: 'n',
				...
				h('section', { key: 'diag', className: 'dsa-card' }, [
					...
					h('div', { key: 'rows', className: 'dsa-rows' }, [
						...
						// 通知最上方那一行到底显示什么：它由系统按 AUMID 的显示名渲染，
						// 因此「同步成功了没有」必须看得见——否则用户只会看到「改了没生效」。
						h('div', { key: 'displayname', className: 'dsa-row' }, [
							h('span', { key: 'l', className: 'dsa-label' }, tx('aumidDisplayName')),
							h('code', { key: 'v', className: 'dsa-value' },
								aumid.titleInAppName === true
									? String(aumid.displayName || '') + tx('aumidSynced')
									: tx('aumidNotSynced')),
						]),
		...
		function apply(ctx) {
			...
			// 两份字典**只有 `title` 不同**（中文「会话通知」/ 其余语言「Session Alert」），
			// 理由见 TEXTS 与 TEXTS_EN_OVERRIDES 上方的注释。
			try {
				var locale = ctx.get('locale')
				if (locale !== undefined && locale !== null && typeof locale.register === 'function') {
					var textsEn = Object.assign({}, TEXTS, TEXTS_EN_OVERRIDES)
					ctx.effect(function () {
						var disposers = []
						try {
							disposers.push(locale.register(LOCALE_NS, 'zh', TEXTS))
							disposers.push(locale.register(LOCALE_NS, 'en', textsEn))
						} catch (error) {
