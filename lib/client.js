...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			//
			// **必须记录是否真的注入成功。** 用户两次反馈「样式没变化」，而文件层面
			// （源 → bundle → 重启时机）全部正确——那就只剩「注入没发生」或「注入被覆盖」
			// 两种可能，而这两种在页面上都不会报错。因此这里把结果写到控制台，
			// 使「有没有注入」从猜测变成一个可读的事实。
			try {
				if (typeof styles !== 'undefined' && styles !== null && typeof styles.insert === 'function') {
					ctx.effect(function () {
						const dispose = styles.insert(STYLES)
						logDebug(`样式已注入（${STYLES.length} 字符）`)
						return dispose
					}, 'dsh-session-alert: styles')
				} else {
					logDebug(`styles 内置不可用（typeof styles = ${typeof styles}），设置页将没有自定义样式`)
				}
			} catch (error) {
				logDebug(`样式注入失败：${error !== null && error !== undefined && error.message ? error.message : String(error)}`)
			}
