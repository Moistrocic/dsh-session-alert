 ...
 * `titleInAppName` 为真时，**自有 AUMID 那一条不写标题行**：那一行的内容已经由 Windows
 * 用 AUMID 的显示名显示在最上方了（`scripts/set-aumid-display-name.ps1` 把它同步成配置里的
 * 通知标题），再写一遍就是同一句话出现两次。后备 AUMID（Windows PowerShell）**始终保留
 * 标题行**——它的应用名是「Windows PowerShell」，去掉标题会让通知完全看不出是谁发的。
 *
 * **刻意不写任何 `<image>` 元素。** 通知左侧那个图标由**应用身份**提供（注册表的 `IconUri`
 * 与开始菜单快捷方式的图标）。toast XML 里再写一个 `appLogoOverride` 会在**正文里多出一个
 * 图标**——实测用户一眼就看出来了：「标题的图标正常，内容为什么还有一个图标？」
 * 少写一个元素还顺带避开「Win32 应用能不能用 `file:///` 图片」这类坑。
...
export function buildToastScript({
  ...
  const senderLines = senders.map((sender) => {
    ...
    const withTitle = !(titleInAppName === true && sender.aumid === PRIMARY_AUMID)
    return `    @{ aumid = (ConvertFrom-B64Text '${utf8Base64(sender.aumid)}'); code = ${code}; withTitle = $${withTitle ? 'true' : 'false'} }`
  })
  ...
  return [
    ...
    // **点卡片不触发任何逻辑**：`activationType="system"` 让系统只是收下这条通知，
    // 不去激活应用。此前不写这个属性时，点卡片会按 AUMID 激活应用身份——也就是拉起
    // 开始菜单快捷方式指向的那个「惰性命令」（实测是一次隐藏的 PowerShell 进程）。
    // 所有逻辑都放在明确的按钮上，卡片本身只用来确认收到。
    "    $toast.SetAttribute('activationType', 'system')",
    ...
    // 这里**不放** `<image placement="appLogoOverride">`：应用图标已经由应用身份提供，
    // 再写一个会在正文里多出一个图标（用户实测反馈）。
    '    if ($WithTitle) {',
    ...
    '        $document = New-ToastDocument -Title $title -Body $body -WithTitle ([bool]$sender.withTitle) -WithSound $withSound -Urgent $urgent -Actions $actions',
 ...
 * @param successNotes - 退出码 → 成功说明的映射；不在表里的码即失败。默认是投递码表。
 * @returns `{ ok, code?, note?, error? }`。
 */
function runPowerShell(executable, script, successNotes = DELIVERY_CODES) {
 ...
 * `titleInAppName` 表示自有 AUMID 的显示名已经等于标题（见 {@link buildToastScript}）。
...
export async function playChime({ source = 'system', filePath = '' } = {}) {
  ...
  let last = failure('没有可用的 PowerShell 解释器')
  for (const executable of powershellCandidates()) {
    last = await runPowerShell(executable, script, { 0: '铃声已播放' })
    if (last.ok) return last
    if (!isMissingInterpreter(last)) return last
  }
  return last
 ...
 * 真机碰运气。
 *
 * 抑制（decision 15）：`suppressed: true`（别名 `suppressCard: true`）时卡片被扣下，
 * **铃声仍然响**，但抑制不绕过限流。抑制只对 desktop 端有意义，因此是否抑制由调用方
 * 按端别与实测焦点决定。
 */
export class AlertDispatcher {
  ...
  constructor(options = {}) {
    ...
    /**
     * 交给投递的**额外字段**（宿主才知道的东西，例如「署名已由应用名承担」与图标路径）。
     *
     * 为什么是这个形状，而不是让宿主包一层 `send`：包一层就得在宿主里 import 投递函数，
     * 而那条路**只在真的投递时才执行**——离线测试根本走不到。本项目已经因此栽过一次：
     * 宿主包的那层 `send` 引用了没导入的 `sendWindowsToast`，于是每次投递都在运行时抛
     * `sendWindowsToast is not defined`，而全部离线测试是绿的、界面上还报「已发送」。
     * 交一个纯数据回调，默认投递链原样保留，调用方不需要 import 任何东西。
     */
    this.sendExtras = typeof options.sendExtras === 'function' ? options.sendExtras : () => ({})
   ...
   *
   * @param bypass - 预览投递：**不占用限流窗口、也不更新「该场景上次提醒时刻」**。
   * 否则用户点两下预览就能把真实提醒挤掉，而那是他不会预料到的后果。
   * 但活动列表照记（`reason: 'preview'`）——「刚才那条到底发出去没有」必须能查。
   */
  deliver(config, scenario, body, now, reason, durationSeconds, actions, bypass) {
    const entry = {
      at: now,
      time: clockTime(now),
      scenario,
    ...
    // 额外字段在**这里**合并进投递请求：宿主只提供数据，投递链本身不被打包一层。
    let extras = {}
    try {
      const fromHost = this.sendExtras()
      if (fromHost !== null && typeof fromHost === 'object') extras = fromHost
    } catch (error) {
      this.log('warn', `投递额外字段取值失败（按无额外字段继续）：${error && error.message ? error.message : String(error)}`)
    }

    /**
     * 投递的真实结果。
     *
     * `deliver` 是同步返回的（`sent: true` 只表示「已交给投递链」），真正的成功/失败要等
     * 投递函数回来。把这条链的 promise 记在条目上，调用方（设置页的预览接口）才能**等**它，
     * 而不是拿「已交给投递链」当成功报给用户——那正是本轮把失败报成「已发送」的原因。
     */
   ...
   *
   * `bypass: true` 是**设置页按下去的那次预览**专用的：绕过去重、场景开关、场景最小间隔、
   * 限流与抑制，并且**不占用限流窗口**。理由是这类动作由用户当场发起，任何静默扣下都会
   * 让他以为功能坏了；而它也不该把一个真实的提醒挤掉。总开关仍然有效——「别给我发通知」
   * 是明确的意图，预览也该如实回一句「通知已关闭」而不是照发。
   *
   * @returns `{ sent, reason, coalescing? }`。
   */
  dispatch({ scenario, body, dedupeKey, focused, suppressed, suppressCard, actions, bypass } = {}) {
    const config = this.getConfig()
    ...
    const scenarioConfig = config.scenarios !== undefined && config.scenarios !== null
      ...
      : undefined
    // 预览不看场景开关：用户点的是「这一种长什么样」，关着也该看得到。
    if (bypass !== true
      && (scenarioConfig === undefined || scenarioConfig === null || scenarioConfig.enabled !== true)) {
    ...
    const durationSeconds = clampDurationSeconds(
      scenarioConfig !== undefined && scenarioConfig !== null
        ? scenarioConfig.durationSeconds
        : DEFAULT_DURATION_SECONDS,
    )
    ...
    const holdCard = bypass !== true && (suppressed === true || suppressCard === true
    ...
    const minIntervalMs = typeof scenarioConfig === 'object' && scenarioConfig !== null
      && typeof scenarioConfig.minIntervalSeconds === 'number'
      ? scenarioConfig.minIntervalSeconds * 1000
      : 0
    const lastAt = this.lastByScenario.get(scenario)
    if (bypass !== true && minIntervalMs > 0 && lastAt !== undefined && now - lastAt < minIntervalMs) {
      return this.block(scenario, config, text, now, 'scenario-interval', durationSeconds, buttons)
    }

    if (bypass !== true && config.rateLimit !== undefined && config.rateLimit.enabled === true) {
      this.pruneWindow(now, config.rateLimit.windowSeconds * 1000)
      if (this.windowHits.length >= config.rateLimit.max) {
    ...
    return this.deliver(config, scenario, text, now, bypass === true ? 'preview' : 'sent', durationSeconds, buttons, bypass === true)
  }
