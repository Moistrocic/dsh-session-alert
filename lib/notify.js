 ...
 *
 * @param request - 标题、正文、声音、发送者序列、时长、按钮、应用图标路径。
 ...
 *
 * `iconPath` 是本地图片文件，写成 toast 的 `appLogoOverride`。它在**两条发送者上都写**：
 * 图标是这条通知自己的属性，与署名由谁承担无关。留空则不写该元素——缺图标文件时宁可不写，
 * 也不要在通知上留一个裂图占位。
...
export function buildToastScript({
  ...
  const icon = typeof iconPath === 'string' ? iconPath.trim() : ''
  ...
  return [
    ...
    // `file:///` 是 Win32 应用引用本地图片的写法（`ms-appx:` 只对打包应用有效）。
    `$iconUri = ${icon.length === 0 ? "''" : `'file:///${icon.replace(/\\/g, '/')}'`}`,
    ...
    '        $document = New-ToastDocument -Title $title -Body $body -WithTitle ([bool]$sender.withTitle) -IconUri $iconUri -WithSound $withSound -Urgent $urgent -Actions $actions',
 ...
 * 验证。**永不 reject。**
 *
 * @param request - 标题、正文、是否带声音、可选显式发送者、显示时长、按钮、图标；
