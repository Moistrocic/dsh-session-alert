...
/**
 * 构造审批按钮要激活的 URL（批准 / 拒绝）。
 *
 * 与 {@link protocolUrl} 同一个方案，因此走的还是同一个启动器——区别只在路径与参数：
 * 启动器看到 `decide` 就**不发窗口指令**，而是把决定 POST 回宿主。
 *
 * `token` 是这次审批专用的一次性令牌（见 `lib/decisions.js`）：它是这条通道上唯一的凭据。
 * 卡片上的按钮参数是本机任何进程都能读到的（通知历史就在用户目录里），所以令牌
 * 只能做到「一次性 + 有期限」，做不到密码学意义上的保密——这也是为什么**默认永远是
 * 没有决定**：拿不到决定就交回界面，绝不替用户放行。
 *
 * @param token - 一次性令牌。
 * @param decision - `'allow'` 或 `'deny'`。
 * @returns 可放进 toast action `arguments` 的 URL。
 */
export function decisionUrl(token, decision) {
  return `${PROTOCOL_SCHEME}://decide/?token=${encodeURIComponent(String(token))}`
 ...
 * Host 据此渲染模板。
 *
 * `defaultMinIntervalSeconds` / `defaultDurationSeconds` 是**每个场景各自的默认值**。
 * 放在这里而不是在 `defaultConfig()` 里写一堆 `id === 'xxx' ? … : …`，是因为这两项
 * 本来就是「这个场景该多久提醒一次、通知该留多久」的一部分：读场景定义时就能看到，
 * 加场景时也不会漏。改默认值只改这一处。
 *
 * **默认正文不含 `{workspace}`**（2026-09-25 用户要求）：通知最上方那一行已经是配置的
 * 通知署名，正文里再出现一个「工作区目录名」既重复又容易被误认成插件的名字
 * （实测用户就把它当成了署名）。`{workspace}` 仍是可用变量，谁需要谁自己加回模板。
 */
export const SCENARIOS = [
  {
    ...
    defaultBody: '{session} 已完成一轮，等待你的下一步指令。',
    defaultMinIntervalSeconds: 0,
    // 30 秒：一轮结束是最常见的提醒，而它对应的动作是「回到窗口看一眼」，
    // 留久一点才不至于在用户切回来的路上就消失。
  ...
  },
  {
    ...
    defaultBody: '{session} 正在等待你的回答：{summary}',
    defaultMinIntervalSeconds: 0,
    // 0 = 常驻：这类事件**必须由人处理**才能继续，等它自己消失没有意义。
  ...
  },
  {
    ...
    defaultBody: '{session} 等待你的授权：工具 {tool}',
    // 0：同理，工具正卡在等待授权上，通知消失只会让用户以为没事了。
    defaultMinIntervalSeconds: 0,
    defaultDurationSeconds: 0,
  },
  {
    ...
    placeholders: ['workspace', 'session', 'summary', 'time'],
    defaultBody: '{session} 执行出错：{summary}',
    defaultMinIntervalSeconds: 0,
    // 0：出错信息通常需要照着排查，不该自己消失。
    defaultDurationSeconds: 0,
  },
]
 ...
 *    强行套用会误抑制。
 *  - 四个场景默认全开，且**最短间隔一律 0**（不限）：限流那一层已经负责防刷屏，
 *    场景级的冷却只会让该来的提醒晚到。需要时由用户在设置页按场景调。
 *  - 显示时长**按场景给**（见 `SCENARIOS`）：需要人动手的三类（提问 / 授权 / 出错）
 *    默认 0 即常驻，轮次结束默认 30 秒。
 */
export function defaultConfig() {
  ...
  for (const scenario of SCENARIOS) {
    scenarios[scenario.id] = {
      ...
      minIntervalSeconds: typeof scenario.defaultMinIntervalSeconds === 'number'
        ? scenario.defaultMinIntervalSeconds
        : 0,
      /** 通知显示时长（秒）。0 表示常驻，直到用户手动关闭。 */
      durationSeconds: typeof scenario.defaultDurationSeconds === 'number'
        ? scenario.defaultDurationSeconds
