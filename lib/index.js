...
export function apply(ctx, config) {
  ...
  async function dispatch(scenarioId, sessionId, extra, meta) {
    ...
    //
    // 这里只放「打开会话」一个按钮。ADR 0002 规定了三种卡片形态（无按钮 / 一个确认
    // 按钮 / 审批的批准与拒绝两个按钮），本轮先落地最基础的一种——把链路打通，
    // 再按形态补齐。普通卡片的按钮**只确认收到、不跳转**；跳转由点击卡片本身承担，
    // 因此按钮文案用「知道了」而不是「打开会话」。
    const actions = [{
      content: '知道了',
    ...
    const verdict = result.sent === true
      ? (suppressed ? 'sent:card-withheld' : 'sent')
