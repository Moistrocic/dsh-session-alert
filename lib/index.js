...
export function apply(ctx, config) {
  ...
  ctx.on('approval/request', async function (request, next) {
    ...
    // 界面那侧的决定：**立刻启动，不等通知发完**。用户可能正看着界面，让提示晚出现
    // 几秒是没有理由的；谁先给出决定就采纳谁。
    //
    // `next()` 必须**同步**调用（而不是排进微任务）：它是「本监听器不否决后续链」的直接
    // 证据——事件接线检查在调用后立刻数 `next` 被调用了几次，为 0 就说明用户将看不到
    // 界面上的提问/审批。
    let fromUi
    try {
      fromUi = Promise.resolve(next())
    } catch (error) {
      report('warn', `启动界面侧审批失败（决定权仍在界面）：${errorText(error)}`)
      fromUi = Promise.resolve('unavailable')
    }
    ...
    try {
      ...
      // 用户中止请求时令牌立刻作废（迟到的点击不能再落成一个决定）。
      const signal = payload.signal
      const onAbort = () => armed.dispose()
      if (signal !== null && signal !== undefined && typeof signal.addEventListener === 'function') {
        signal.addEventListener('abort', onAbort, { once: true })
      }
      try {
        // **谁先给出决定就采纳谁。**
        // 通知那侧超时或失效时，它的 promise 永远不会兑现 —— 于是这条 race 只可能由
        // 界面那侧结束。这就是「沉默绝不是同意」在实现里的样子：没有默认放行，
        // 也没有默认拒绝，只有「交回界面」。
        return await Promise.race([armed.promise, fromUi])
      } finally {
        armed.dispose()
        if (signal !== null && signal !== undefined && typeof signal.removeEventListener === 'function') {
