...
export function apply(ctx, config) {
  ...
  /**
   * 通知上的应用图标（toast 的 `appLogoOverride`）。
   *
   * 用插件自带的 PNG——它由 `npm run build:icon` 从 DeepSeek Harness 自己的图标生成
   * （见 `scripts/build-notification-icon.ps1`）。文件不在就当没有：宁可不写这个元素，
   * 也不要在通知上留一个裂图占位。
   */
  const notificationIconPath = join(PACKAGE_ROOT, 'assets', 'notification-icon.png')
  const hasNotificationIcon = existsSync(notificationIconPath)

  const dispatcher = new AlertDispatcher({
    getConfig: () => current,
    onEvent: (level, message) => report(level, message),
    // **不在这里包一层 `send` 去注入这两个字段。**
    //
    // 曾经就是这么写的：`send: (request) => sendWindowsToast({ ...request, titleInAppName, iconPath })`。
    ...
    sendExtras: () => ({
      titleInAppName,
      iconPath: hasNotificationIcon ? notificationIconPath : '',
    }),
  })
  ...
  ctx.on('approval/request', function (request, next) {
    try {
      ...
      // 默认模板只用 `{tool}`，因此这条只在用户改了模板时才有可见差别——但摘要字段
      // 本来就该填「有意义的那句话」，而不是把工具名重复一遍。
      ...
      void dispatch('approval', sessionId, {
        tool: truncate(tool),
