 ...
 * 设置页预览正文的长度上限（字符）。
 *
 * 这是环回路由，正文来自本插件自己的设置页，因此不是安全边界——它挡的是**病态输入**：
 * 一个被粘贴进来的超长模板会变成一条塞满整屏的通知，而 Windows 对 toast 文本本身
 * 也有长度限制，超了会被静默截断。宁可如实回绝并说明原因。
 */
const PREVIEW_BODY_LIMIT = 400

/**
 * 预览接口等投递真实结果的时限（毫秒）。
 *
 * 投递链要起一个 Windows PowerShell、再回读操作中心历史，正常在 1–2 秒内回来。
 * 超时就回「已发出、结果未知」而不是报失败——**超时不等于失败**，而把慢当成失败
 * 会让人去查一个没坏的地方。
 */
const PREVIEW_RESULT_TIMEOUT_MS = 8000

/**
 * 「先只等通知」的宽限期（毫秒）。
 *
 * 通知上的批准/拒绝与界面提示**只能有一个是活的**：界面提示一旦弹出来，就没有任何东西
 * 能把它收回（它是另一个应答者在等它自己的答案），于是通知先答之后它会一直挂着——
 * 用户看到操作已经成功、输入区上方却还挂着一个可点的「批准」弹框，分不清是「没生效」
 * 还是「没更新」。实测就是这么被反馈的。
 *
 * 所以顺序改成：卡片带按钮发出去 → 只等通知 → 宽限期到才把决定权交给界面。
 * 卡片被抑制（用户正看着窗口）时不存在这个冲突：那条路直接给界面提示。
 *
 * 取舍：窗口没聚焦但人在机器前时，界面提示会晚这么久出现。20 秒是折中——通知就在眼前时
 * 它足够快，而人不在机器前时这点延迟无感。
 * `DSH_SESSION_ALERT_NOTIFY_GRACE_MS` 可覆盖（测试压到 0，不必等 20 秒）。
 *
 * @returns 宽限期毫秒数。
 */
function readNotificationGraceMs() {
  const raw = Number.parseInt(String(process.env.DSH_SESSION_ALERT_NOTIFY_GRACE_MS ?? ''), 10)
  if (Number.isFinite(raw) && raw >= 0) return raw
  return 20_000
}

/**
 * 用户**已经在 DSH 窗口前**时的宽限期（毫秒）。
 *
 * 那种情形下界面提示才是他要用的东西，通知只该有一个很短的机会抢答。**用户明确要求
 * 「点击跳转后应当立刻刷新，而不是等待」**，因此这里取 1 秒：效果上等同于立刻
 * （请求一到，界面提示随即出现），只保留一个不至于把通知按钮变成摆设的最小窗口。
 *
 * 为什么不能做到「既立刻刷新、通知按钮又能点」：那需要区分「点了批准」与「点了跳转」，
 * 而唯一的区分依据是按钮自己的 URL（令牌 + 意图）。跳转按钮目前不带令牌（不带 = 不消费
 * 决定），所以 Host 无法知道用户点了跳转 —— 只能靠定时或焦点信号猜。**正解是让跳转按钮
 * 也带上令牌与 `decision=ui`，由启动器 POST 回来**；那要改 C# 启动器并重建（当时没有 shell）。
 */
const FOCUSED_GRACE_MS = 1000
...
export function apply(ctx, config) {
  ...
  /**
   * 自有 AUMID 的显示名是否已经等于配置里的通知标题。
   *
   * 通知最上面那一行（应用名）由 Windows 按 AUMID 的显示名渲染，**不是** toast XML 决定的。
   * 因此「让那一行显示我的通知标题」只能在注册层面做，而且做了之后 toast 里就**不该再写
   * 标题行**——否则同一句话会出现两次（`titleInAppName` 一路传到 `buildToastScript`）。
   *
   * 它默认 `false`：同步失败时仍保留标题行。宁可重复一句话，也不要让通知完全没有标题
   * ——后备 AUMID 那条路（应用名是「Windows PowerShell」）就更是如此。
   */
  let titleInAppName = false

  /**
   * 通知左侧那个图标**不在这里管**。
   *
   * 它由**应用身份**提供：注册表的 `IconUri` 与开始菜单快捷方式的图标，都指向
   * `npm run build:icon` 从 DeepSeek Harness 自己的图标生成的那份 ICO
   * （见 `scripts/build-notification-icon.ps1` 与 `scripts/register-aumid.ps1`）。
   *
   * 曾经在 toast XML 里加过 `<image placement="appLogoOverride">`，结果是**正文里多出一个
   * 图标**（用户实测反馈：「标题的图标正常，内容为什么还有一个图标？」）。那个元素已移除，
   * 这里也不再传图标路径——少写一个元素，也顺带避开「Win32 应用能不能用 `file:///`」的坑。
   */
  ...
  const dispatcher = new AlertDispatcher({
    ...
    // **不在这里包一层 `send` 去注入字段。**
    //
    // 曾经就是这么写的：`send: (request) => sendWindowsToast({ ...request, titleInAppName })`。
    // 而 `sendWindowsToast` **没有从 notify.js 导入**（此前用不到——分发器内部默认就用它）。
    // 于是每次投递都在运行时报 `sendWindowsToast is not defined`，被投递链记成 failed，
    // **而那时所有离线测试都是绿的**：没有一条测试真的走完投递（真的走就会弹通知）。
    // 现在改成向分发器交一个「额外字段」回调，默认投递链原样保留，也不需要这个 import。
    sendExtras: () => ({ titleInAppName }),
  })
  ...
  if (process.platform === 'win32' && !aumid.registered) {
    void registerAumid()
      .then((registered) => {
        ...
        // **注册成功之后要再同步一次署名。** 挂载时如果还没注册，同步会直接跳过
        // （未注册时不该去写显示名），于是全新安装第一次跑就会漏掉这一步——
        // 通知最上方那一行会停在注册名上，而用户以为改标题没生效。
        if (registered) {
          void syncAumidDisplayName().catch((error) => report('warn', `通知署名同步异常：${errorText(error)}`))
        }
      })
  ...
  /**
   * 把 AUMID 的显示名同步成配置里的通知标题——**通知最上面那一行就是它**。
   *
   * 为什么需要这一步：`<text id="1">` 只是正文上方那一行，而 Windows 在最顶上还会用
   * AUMID 的显示名显示「应用名」。用户看到的「通知标题」其实是后者。实测（只改注册表值、
   * 不动开始菜单快捷方式）下一条通知的应用名就变了。
   *
   * 刻意**不重命名快捷方式**：快捷方式的名字同时是 `isAumidRegistered()` 的判据
   * （`<AUMID>.lnk` 是否存在），改名会让插件误判「未注册」而退回后备 AUMID——
   * 那会把横幅的来源换成 Windows PowerShell，比原来的问题更糟。
   *
   * 幂等由脚本保证（值相同就直接退出），而这里再加一道「与上次同步值相同就跳过」，
   * 免得每次保存配置都起一个 PowerShell 进程。
   */
  /**
   * 待决审批的令牌表（见 `lib/decisions.js`）。
   *
   * **只在内存里，且绝不出现在 `/state` 里**：`/state` 是本机无凭据可读的，
   * 把令牌放进去等于把「放行一次工具调用」的入口挂出去。
   */
  const decisions = new DecisionStore({
    onEvent: (level, message) => report(level, message),
    now: () => Date.now(),
  })
  ctx.effect(() => () => { decisions.pending.clear() }, `${name}: decisions`)

  let syncedDisplayName = null
  async function syncAumidDisplayName() {
    if (process.platform !== 'win32') return
    if (!aumid.registered) {
      // 未注册时不去写显示名：那会让「系统认识这个应用」这件事变得含糊，
      // 而横幅能否出现本来就看快捷方式在不在。
      return
    }
    const wanted = String(current.title ?? '').trim()
    if (wanted.length === 0 || wanted === syncedDisplayName) return
    const script = join(PACKAGE_ROOT, 'scripts', 'set-aumid-display-name.ps1')
    if (!existsSync(script)) {
      report('warn', `未找到 ${script}，通知最上方仍会显示 AUMID 的注册名（通知本身不受影响）`)
      return
    }
    const outcome = await runPowerShellFile(pickPowerShellInterpreter(), [
      '-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass',
    ...
    if (outcome.ok) {
      syncedDisplayName = wanted
      titleInAppName = true
      report('info', `通知署名已同步为「${wanted}」`)
    } else {
      // 同步失败**不是**致命问题：标题行还在（titleInAppName 保持 false），
      // 只是最上方那一行仍显示注册名。如实告警，不假装成功。
      titleInAppName = false
      report('warn', `通知署名同步失败：${outcome.error ?? `退出码 ${outcome.code}`}（最上方那一行仍显示注册名，通知本身正常）`)
    }
  }

  void syncAumidDisplayName().catch((error) => {
    report('warn', `通知署名同步异常：${errorText(error)}`)
  })
  ...
  /**
   * 等待「desktop 端获得焦点」的等待器集合（见 {@link desktopFocusSignal}）。
   *
   * 只在审批那条路上用：通知已发出、界面提示**还没**给（这是刻意的，见 ADR 0007），
   * 而用户点「跳转到Harness」时不会消费令牌 —— 他恰恰要去界面处理，所以窗口一获得焦点
   * 就该立刻把提示交出去，而不是等满宽限期。
   */
  const desktopFocusWaiters = new Set()

  /**
   * 各端上报的**样式状态**（客户端从浏览器读回的那份）。
   *
   * 为什么 Host 要存这个：设置页的样式失效时**页面上不报任何错**，而「有没有注入」
   * 与「注入了有没有生效」是两件事。上一轮在这上面耗了整整一个迭代周期，因为唯一的
   * 判据是「重启后看看好不好看」——那不是判据，那是印象。
   *
   * 客户端每次上报都带一份实测读数（标签数、CSS 字符数、浏览器解析出的规则数，
   * 以及 `getComputedStyle` 读回的计算值），Host 存下来并经 `/state` 暴露，
   * 于是「样式到底生效没有」可以**从外面读**，不必靠猜也不用人眼。
   */
  const clientStyles = new Map()

  /** 记录一次客户端上报。 */
  function noteClientPresence(kind, focused, styles) {
    if (kind !== 'web' && kind !== 'desktop') return
    clientPresence.set(kind, { at: Date.now(), focused: focused === true })
    // 「用户回到 DSH 窗口了」：审批在等通知作答时靠这个信号**提前**把决定权交给界面。
    // 没有它的话，点「跳转到Harness」的用户要干等满宽限期才看到批准提示 —— 实测被反馈
    // 「点了跳转，批准界面延迟至少 5 秒」，而那一刻他正盯着窗口。
    if (kind === 'desktop' && focused === true) {
      for (const waiter of [...desktopFocusWaiters]) waiter()
    }
    // 样式状态只在客户端给了合法对象时更新：缺字段的旧客户端不该把已有读数抹掉。
    if (styles !== undefined && styles !== null && typeof styles === 'object' && !Array.isArray(styles)) {
      clientStyles.set(kind, { at: Date.now(), report: styles })
    }
  }
  ...
  /**
   * desktop 端此刻是否在前台（已剔除过期项）。
   *
   * **不要与抑制策略混为一谈**：抑制还要看 `suppressWhenFocused` 配置。
   * 这个函数只回答「用户此刻是不是在 DSH 窗口前」，两个用途都要它：
   * 决定要不要抑制卡片，以及判断「卡片发出时用户是否已经在前台」。
   */
  function isDesktopFocused() {
  ...
  /**
   * 「desktop 端获得焦点」的信号。**用于审批在等通知时提前交接。**
   *
   * 为什么需要它：审批已发出通知时**不**提前给界面提示（否则通知先答之后，界面那个提示
   * 收不回来——见 ADR 0007）。但用户点「跳转到Harness」不会消费令牌，他就是要回界面处理，
   * 那一刻界面**必须**已经有提示；只靠宽限期他得干等最多 20 秒（实测被反馈
   * 「点了跳转，批准界面延迟至少 5 秒」）。因此窗口一获得焦点就立刻交接。
   *
   * 客户端半边的焦点上报是**事件驱动**的（`focus`/`blur` 监听后立即 fetch），
   * 所以这个信号的延迟是几十毫秒级，不是心跳周期。
   *
   * @returns `{ promise, cancel }`；等待结束**必须** `cancel()`，否则等待器会留在集合里。
   */
  function desktopFocusSignal() {
    const focusedNow = () => {
      const record = clientPresence.get('desktop')
      return record !== undefined && record.focused === true
        && Date.now() - record.at <= CLIENT_PRESENCE_TTL_MS
    }
    let waiter = null
    const promise = new Promise((resolve) => {
      if (focusedNow()) { resolve('already-focused'); return }
      waiter = () => { desktopFocusWaiters.delete(waiter); resolve('focused') }
      desktopFocusWaiters.add(waiter)
    })
    return { promise, cancel: () => { if (waiter !== null) desktopFocusWaiters.delete(waiter) } }
  }
   ...
   * 分发一个场景，并把判决记入信号表。
   *
   * `meta.bodyOverride` 只给**设置页按下去的那次预览**用：正文由用户当场看到的那份预览
   * 直接给出，因此不允许再渲染模板——他要看到的是「我看到的这条」，而不是「Host 用别的
   * 变量再渲染一遍」。其余部分（按钮形状、判决串、活动列表）与真实事件完全同路，
   * 这样预览才作数。
   *
   * `meta.bypass` 见 `AlertDispatcher.dispatch` 的说明：绕过去重/限流/抑制，但不绕总开关。
   */
  async function dispatch(scenarioId, sessionId, extra, meta) {
    ...
    const preview = typeof meta.bodyOverride === 'string'
    const info = await describeSession(sessionId)
    if (!preview && current.onlyRootSessions && !isRootSession(info, sessionId)) {
    ...
    const body = preview ? meta.bodyOverride : renderTemplate(scenarioConfig.body, vars)
    ...
    //
    // 两种形状（2026-09-25 用户要求，取代 ADR 0002 的「点击卡片本身跳转」）：
    //   普通场景  -> 一个「跳转到Harness」按钮
    //   审批场景  -> 「批准」「拒绝」「跳转到Harness」三个按钮（顺序即声明顺序）
    //
    // **卡片本身不再承载任何动作**（toast 用 `activationType="system"`，见 notify.js）：
    // 点卡片只确认收到，所有逻辑都在明确的按钮上。
    //
    // 审批按钮为什么现在可以是真的：DSH 的审批应答者就是 `approval/request` 瀑布监听器
    // （见上面那个监听器），而它**在轮次内、进程内**——ADR 0006 当年否掉的是
    // 「把请求 id 放进 URL、让外部进程稍后回调」那条路（点击时轮次可能已经结束，
    // 决定会落到别的请求上）。现在的做法不经过那条路：令牌**只绑定这一个请求**、
    // 一次性、有期限，点击只是把用户的决定交回**正在等待的这个应答者**。
    const desktopOnline = liveClientKinds().includes('desktop')
    const isApproval = scenarioId === 'approval'
    const jump = { content: '跳转到Harness', arguments: protocolUrl(sessionId), activationType: 'protocol' }
    const decide = meta.decide
    const canDecide = isApproval && desktopOnline && decide !== undefined && decide !== null
    const actions = desktopOnline
      ? (canDecide
        ? [
          { content: '批准', arguments: decisionUrl(decide.token, 'allow'), activationType: 'protocol' },
          { content: '拒绝', arguments: decisionUrl(decide.token, 'deny'), activationType: 'protocol' },
          jump,
        ]
        : [jump])
      : []
    ...
    const result = dispatcher.dispatch({
      ...
      // 预览：抑制不适用——用户就看着设置页点下去的，扣下卡片只会让他以为按钮坏了。
      suppressed: preview ? false : suppressed,
      actions,
      bypass: meta.bypass === true,
    })

    // 判决要如实反映「扣了卡片但响了铃」——这与「根本没发」是两回事，
    // 也与「发了卡片」是两回事。混成一个 'sent' 会让排查时看不出发生过抑制。
    // 同时记下卡片形态：无控件的形态是**设计结果**而不是缺陷，诊断时要能区分。
    // 判决串带上**按钮个数**：卡片形状（几个控件、有没有控件）是要能一眼看出来的诊断信息，
    // 而审批卡片是 3 个按钮、其余是 1 个——合成一个 `card+button` 就分不出来了。
    const shape = actions.length === 0
      ? 'card-only'
      : `card+${actions.length}button${actions.length > 1 ? 's' : ''}`
    const verdict = result.sent === true
      ? (preview ? `preview:${shape}` : (suppressed ? 'sent:card-withheld' : `sent:${shape}`))
      : (result.reason === 'focused-suppressed' ? 'suppressed:chime-only' : `skipped:${result.reason}`)
    noteSignal(meta.source, sessionId, verdict)
    // 按钮数如实回给调用方：审批监听器要靠它确认「这张卡片真的带着按钮发出去了」，
    // 否则令牌会在没人能点的情况下继续有效。
    return { ...result, actions: actions.length }
  }
   ...
   *
   * ## 会话与题目都取自**载荷**，不取自 `this`（这是一个真实缺陷的修法）
   *
   * 第一版写的是 `this.agent.id` 与 `payload.question`，两者都取不到东西，而**页面上
   * 什么都看不出来**：通知照发，只是正文变成 `… · 未知会话 正在等待你的回答：`——
   * 会话名退化成「未知会话」、问题摘要为空。它是靠**真实提问**才暴露的
   * （信号表里 `session` 一栏为空、卡片正文末尾是空的冒号）。
   *
   * 权威依据是发射点的源码（`@deepseek-ai/dsh-user-questions/lib/index.js`）：
   *
   *   `ctx.waterfall(scopeTarget(agent, agent), 'user-questions/request', { ...request, agent }, noAnswerer)`
   *
   * 也就是说 **agent 被并进了载荷**（`{ ...request, agent }`），而 `this` 是作用域对象
   * 而非 Agent 本身。scoped 事件的路由键也印证这一点：
   * `@deepseek-ai/dsh-scope` 的 `scopedSubjectResolvers` 里写着
   * `'user-questions/request': (args) => args[0]['agent']` —— 取的是**第一个参数上的 agent**。
   *
   * **因此这里只读载荷，不读 `this`。** 那一版写 `this.agent.id` 的代码在真机上取到的是
   * 空字符串（信号表里该行的会话栏为空，正是它的痕迹）。加一条 `this.agent` 兜底看似稳妥，
   * 实则是把「未经证实的假设」留在代码里：它永远不会被走到，因而永远不会被发现是错的。
   * 契约说载荷里有 agent，就只认载荷。
   */
  // 注意用**普通函数**而不是箭头函数：`this` 是 cordis 传入的作用域对象。
  ctx.on('user-questions/request', function (request, next) {
    try {
      ...
      // 题目在 `questions[]` 里（**不是**顶层的 `question` 字段）。
      const items = Array.isArray(payload.questions) ? payload.questions : []
      const first = items.length > 0 && items[0] !== null && typeof items[0] === 'object' ? items[0] : {}
      const question = typeof first.question === 'string' && first.question.length > 0
        ? first.question
        : (typeof first.header === 'string' ? first.header : '')
      // 一次问多个时，正文里带一句数量，否则用户会以为只有一个问题。
      const extra = items.length > 1 ? `（共 ${items.length} 个问题）` : ''

      void dispatch('question', sessionId, { summary: truncate(question + extra) }, {
   ...
   *
   * 参数形状同样以 `cordis_inspect_query` 的 Event 目录为准：
   ...
   *
   * 与提问同理，**agent 在载荷里**（`req.agent`），`this` 不是 Agent，因此只读载荷。
   * （`toolName` 那一处第一版就写对了，因为契约里字段名直观；会话那一处写错了，
   *   因为「`this` 是 Scoped<Agent>」这句话容易读成「`this.agent` 就是那个 Agent」。）
   */
  ctx.on('approval/request', async function (request, next) {
    const payload = request !== null && typeof request === 'object' ? request : {}
    const agent = payload.agent
    const sessionId = agent !== null && agent !== undefined && typeof agent.id === 'string'
      ? agent.id
    ...
    // 授权理由优先取本地化后的那句（`displayReason`），否则退到 `reason`，再退到工具名。
    const display = payload.displayReason !== null && typeof payload.displayReason === 'object'
      ? payload.displayReason
      : null
    const reason = display !== null
      ? (typeof display.zh === 'string' ? display.zh : (typeof display.en === 'string' ? display.en : ''))
      : (typeof payload.reason === 'string' ? payload.reason : '')

    const notificationGraceMs = readNotificationGraceMs()

    // 这个请求对应的待决令牌。**只有在卡片真的带着按钮发出去之后**它才有效——
    // 卡片被抑制（用户正看着界面）或没发出去时，令牌立刻作废：按钮都不在，
    // 没人能点，留着它只会让一个拿得到令牌的进程凭空获得放行能力。
    const armed = decisions.open({ sessionId, toolName: tool, callId: payload.callId })

    /**
     * 把决定权交回界面（`next()`）。
     *
     * **不要提前调用它。** 实测教训：通知与界面提示同时启动、谁先答采纳谁，
     * 结果是**通知先答之后，界面那个提示永远不会被收回**——用户看到操作已经成功，
     * 输入区上方却还挂着一个可点的「批准」弹框，分不清是「没生效」还是「没更新」。
     * 界面提示是另一个应答者在等它自己的答案，没有任何东西会告诉它「已经有人答过了」。
     * 因此改成**顺序**：先只让通知作答，超时才把决定权交给界面（见下面的宽限期）。
     */
    const handOverToUi = () => {
      try {
        return Promise.resolve(next())
      } catch (error) {
        report('warn', `启动界面侧审批失败（决定失据，按不可用结算）：${errorText(error)}`)
        return Promise.resolve('unavailable')
      }
    }

    try {
      const result = await dispatch('approval', sessionId, {
        tool: truncate(tool),
        summary: truncate(reason.length > 0 ? reason : tool),
      }, {
        source: 'approval/request',
        dedupeKey: `approval\u0000${sessionId}\u0000${tool}`,
        // 三个按钮：批准 / 拒绝 / 跳转到 Harness（顺序即声明顺序，从左到右）。
        decide: armed,
      })

      // 卡片没带按钮发出去（被抑制／被跳过）：界面上直接给提示，通知那侧不存在。
      if (result === undefined || result === null || result.sent !== true || result.actions !== 3) {
        armed.dispose()
        return handOverToUi()
      }
      ...
      let onAbort = null
      let graceTimer = null
      // 卡片**发出时**用户已经在前台 → 这次焦点信号**不参与**交接。
      //
      // 否则卡片会在出生瞬间被判成「用户要去界面」而被作废 —— 实测反馈原话：
      // 「DSH 在前台时弹出的通知，点批准依旧无效」。只有「发出时不在前台、之后才获得
      // 焦点」才算一次真正的交接（那正是点「跳转到Harness」的情形）。
      const alreadyFocused = isDesktopFocused()
      const focus = alreadyFocused ? null : desktopFocusSignal()
      /**
       * 本次的实际宽限期。
       *
       * 人在窗口前 → 只留很短一瞬给通知抢答（界面提示随即出现，他不必等）；
       * 人在别处 → 保留长宽限期（通知是主要入口，20 秒足够他看见并点）。
       *
       * 上一版把「已在前台」当成「不启用焦点交接」，结果前台用户点「跳转到Harness」
       * 之后要等满 20 秒才看到批准提示 —— 实测被反馈「跳转到 DSH 后又不显示批准界面了」。
       * 该分的是宽限期长短，不是要不要交接。
       */
      const graceMs = alreadyFocused ? Math.min(notificationGraceMs, FOCUSED_GRACE_MS) : notificationGraceMs
      try {
        // **先只等通知**（宽限期见 NOTIFICATION_GRACE 常量）。这段时间里界面不弹提示 ——
        // 于是不会留下一个无人应答的弹框。用户点了通知 → 请求结算，界面上从头到尾没有
        // 提示要收；用户回到窗口 → 立刻交接（焦点信号）；两者都没有 → 宽限期到再交接。
        const outcome = await new Promise((resolve) => {
          let settled = false
          const finish = (value) => { if (!settled) { settled = true; resolve(value) } }
          armed.promise.then((decision) => finish({ kind: 'notification', outcome: decision }))
          // **宽限期长短取决于「他此刻在不在这个窗口前」。**
          //
          // 人在窗口前（例如 `suppressWhenFocused: false`，前台照样发卡片）→ 界面提示就是
          // 他要用的东西，只给一个很短的窗口让通知抢答，然后立刻交接；否则他会盯着窗口
          // 等满长宽限（实测被反馈「跳转到 DSH 后不显示批准界面」——那正是长宽限期的尾巴）。
          // 人在别处 → 通知才是主要入口，保留长宽限期。
          graceTimer = setTimeout(() => finish({ kind: 'grace' }), graceMs)
          if (focus !== null) focus.promise.then(() => finish({ kind: 'focus' }))
          // 请求被中止（用户取消、轮次结束）：不必再等，交回界面由服务结算成 cancelled。
          onAbort = () => finish({ kind: 'aborted' })
          if (signal !== null && signal !== undefined && typeof signal.addEventListener === 'function') {
            signal.addEventListener('abort', onAbort, { once: true })
          }
        })

        if (outcome.kind === 'notification') return outcome.outcome

        // 宽限期到、用户回到窗口、或请求被中止：**交回界面**，并作废按钮。
        // 作废是刻意的：此后点击那张旧卡片会得到启动器的「已失效，请在界面里作答」——
        // 那是**对的**，因为决定权确实已经在界面上，而界面上正挂着一个活的提示。
        armed.dispose()
        return handOverToUi()
      } finally {
        if (focus !== null) focus.cancel()
        if (graceTimer !== null) clearTimeout(graceTimer)
        armed.dispose()
        if (onAbort !== null && signal !== null && signal !== undefined
      ...
      armed.dispose()
      report('warn', `approval 监听器失败（决定权交回界面）：${errorText(error)}`)
      return handOverToUi()
    }
  }, { global: true })
  ...
  function snapshot() {
    ...
    return {
      ...
      // 待决审批：**只报数量与请求身份，绝不报令牌**（`/state` 本机无凭据可读，
      // 令牌泄露等于把「放行一次工具调用」的入口挂出去）。
      decisions: decisions.snapshot(),
      aumid: {
        primary: aumid.primary,
        registered: aumid.registered,
        // 通知最上面那一行显示的名字。`titleInAppName` 说明它是否已经同步成功——
        // 没同步成功时 toast 里仍保留标题行（宁可重复一句，也不要没有标题）。
        displayName: syncedDisplayName ?? current.title,
        titleInAppName,
      },
      ...
      clients: {
        ...
        // 客户端读回的样式实测。**这是「设置页样式有没有生效」的机器可读答案**——
        // 它来自浏览器（`getComputedStyle` 与 DOM 查询），不来自插件的意图。
        styles: [...clientStyles.entries()].map(([kind, record]) => ({
          kind,
          ageMs: Date.now() - record.at,
    ...
    routes.push(webServer.register({
      ...
      handler: async (request, response) => {
        ...
        if (endpoint === '/client-state' && method === 'POST') {
          ...
          noteClientPresence(body.kind, body.focused, body.styles)
        ...
        if (endpoint === '/config' && method === 'POST') {
          ...
          // 标题可能变了：它是通知最上方那一行（AUMID 显示名），要跟着同步。
          // 不 await：设置页的保存不该等一个 PowerShell 进程；失败会记 warn。
          void syncAumidDisplayName().catch((error) => report('warn', `通知署名同步异常：${errorText(error)}`))
        ...
        // 通知卡片上的「批准 / 拒绝」回来的地方（经协议激活 → 启动器 → 这里）。
        //
        // 这条通道**没有凭据**（本插件的路由本来就不需要凭据），因此决定完全依赖令牌：
        // 128 位随机、一次性、有期限，且只出现在那张卡片的按钮参数里。没有令牌
        // 就没法提交决定——这是「不让本机任意进程放行工具调用」的唯一一道关。
        if (endpoint === '/decide' && method === 'POST') {
          const payload = await readJsonBody(request)
          if (payload === undefined || payload === null || typeof payload !== 'object') {
            sendJson(response, 400, { ok: false, error: '请求体必须是 JSON 对象' })
            return
          }
          const outcome = decisions.decide(payload.token, payload.decision)
          // 失败一律 400 且**不做任何决定**：已用过、已过期、认不出来——都退回界面作答。
          sendJson(response, outcome.ok === true ? 200 : 400, outcome)
          return
        }

        // 设置页「发送这条通知」：发一条**该场景**的预览。
        //
        // 正文由客户端给出（就是它预览区里显示的那一行），因此「看到什么就发什么」是
        // 由构造保证的，而不是靠两处渲染实现碰巧一致。见 `dispatch` 的 `bodyOverride`。
        if (endpoint === '/notify' && method === 'POST') {
          ...
          const scenarioId = typeof payload.scenario === 'string' ? payload.scenario : ''
          if (!SCENARIO_BY_ID.has(scenarioId)) {
            sendJson(response, 400, { ok: false, error: `未知场景：${scenarioId}（可用：${SCENARIO_IDS.join(', ')}）` })
            return
          }
          const text = typeof payload.body === 'string' ? payload.body.trim() : ''
          if (text.length === 0) {
            // 空模板渲染出来的就是空——如实回绝，而不是发一条空白通知。
            sendJson(response, 400, { ok: false, error: '预览为空，没有可发送的内容' })
            return
          }
          if (text.length > PREVIEW_BODY_LIMIT) {
            sendJson(response, 400, { ok: false, error: `预览正文过长（${text.length} > ${PREVIEW_BODY_LIMIT}）` })
            return
          }
          const outcome = await dispatch(scenarioId, '', {}, {
            source: `preview:${scenarioId}`,
            bodyOverride: text,
            bypass: true,
          })
          // **等投递的真实结果再回话。**
          //
          // `outcome.sent === true` 只表示「已交给投递链」；真正的成功/失败在那条链回来时才
          // 知道。此前这里直接按 `sent` 回 `ok: true`，于是宿主每次投递都抛
          // `sendWindowsToast is not defined`、界面上却写着「已发送这条预览」——
          // **把失败报成成功，比不报更糟**。
          const settled = outcome !== undefined && outcome !== null && outcome.done !== undefined
            ? await Promise.race([
              outcome.done,
              new Promise((resolve) => setTimeout(() => resolve(null), PREVIEW_RESULT_TIMEOUT_MS)),
            ])
            : (outcome !== undefined && outcome !== null ? outcome.entry : undefined)
          if (settled === null) {
            // 超时不等于失败：投递链可能只是慢（PowerShell 启动 + 回读操作中心）。
            sendJson(response, 200, { ok: true, pending: true, outcome })
            return
          }
          const entry = settled !== undefined && settled !== null ? settled : outcome.entry
          const delivered = entry !== undefined && entry !== null && entry.ok === true
          sendJson(response, 200, {
            ok: delivered,
            error: delivered ? undefined : String(entry !== undefined && entry !== null && entry.error !== null && entry.error !== undefined ? entry.error : '投递失败'),
            via: entry !== undefined && entry !== null ? entry.via : undefined,
            outcome,
          })
          return
        }
