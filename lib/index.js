...
export function apply(ctx, config) {
  ...
  async function dispatch(scenarioId, sessionId, extra, meta) {
    ...
    // 卡片上的按钮：点击 → 我们的协议 → 无控制台启动器 → 窗口显示到最上层。
    //
    // **端别决定形状，不决定目的地**（ADR 0001）。按钮是 desktop 端的能力：web 端没有
    // 可跳转的应用窗口，给它一个「打开会话」按钮只会是个点了没反应的控件。
    // 因此：有 desktop 端在线才带按钮；只有 web 端或两端都不在线时，发无控件的形态。
    //
    // 这条边界必须守住——为「仅 web 受众」发出的卡片携带控件，等于承诺一件做不到的事。
    const desktopOnline = liveClientKinds().includes('desktop')
    const actions = desktopOnline
      ? [{
        // ADR 0002：普通卡片的按钮**只确认收到、不跳转**；跳转由点击卡片本身承担。
        // 因此文案是「知道了」而不是「打开会话」。
        content: '知道了',
   ...
   * Agent 提问：必须由人作答才能继续。
   *
   * 这是瀑布事件，我们只观察、不干预——不调用 next 也不改结果，因此不会影响
   * 真正的答题流程。
   */
  ctx.on('user-questions/request', (payload) => {
    try {
      if (payload === null || typeof payload !== 'object') return
      const agent = payload.agent
      const sessionId = agent !== null && typeof agent === 'object' && typeof agent.id === 'string' ? agent.id : ''
      const request = payload.request !== null && typeof payload.request === 'object' ? payload.request : {}
      ...
      report('warn', `user-questions 监听器失败：${errorText(error)}`)
    }
  }, { global: true })
   ...
   * 工具等待授权。
   *
   * 同样是瀑布事件，只观察不干预——**插件绝不在这里替用户作答**。ADR 0003 允许的是
   * 「用户点了通知卡片上的按钮」这一路径代答，那是显式的人为动作；自动放行不是本插件
   * 的行为，绝不能顺手加上。
   */
  ctx.on('approval/request', (payload) => {
    try {
      if (payload === null || typeof payload !== 'object') return
      const agent = payload.agent
      const sessionId = agent !== null && typeof agent === 'object' && typeof agent.id === 'string' ? agent.id : ''
      const request = payload.request !== null && typeof payload.request === 'object' ? payload.request : {}
      const tool = typeof request.toolName === 'string'
        ? request.toolName
      ...
      report('warn', `approval 监听器失败：${errorText(error)}`)
    }
  }, { global: true })
  ...
  if (webServer !== undefined && typeof webServer.register === 'function') {
    ...
    routes.push(webServer.register({
      method: 'POST',
      path: `${ROUTE_PREFIX}/client-state`,
      handler: async (request, response) => {
        if (!isLoopbackRequest(request)) { sendJson(response, 403, { ok: false, error: '只接受本机环回请求' }); return }
    ...
    routes.push(webServer.register({
      method: 'POST',
      path: `${ROUTE_PREFIX}/clear-activity`,
      handler: (request, response) => {
        if (!isLoopbackRequest(request)) { sendJson(response, 403, { ok: false, error: '只接受本机环回请求' }); return }
    ...
    report('warn', '未找到 webServer 服务，设置页的读写路由未注册（通知功能不受影响）')
  }
